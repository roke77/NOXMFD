...
| Master Arm ON / OFF, Radar ON / OFF, Engine ON / OFF | plain dedicated pairs — see below | edge |
...
- **WPN page:** always shows **ARM**/**SAFE** (Master Arm) and **A/A**/**A/G** (combat mode)
  controls, independent of these keybinds — no dedicated ALL control there either, same reasoning.
  Master Arm OFF also draws a full-screen X with a SAFE label over the WPN page.
