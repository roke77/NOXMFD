...
// changing the selected HSD range. Holding again while zoomed returns to the normal view.
// The zoom/icon-shrink math lives in the shared services/cursor-zoom.js (rdr.js uses the identical
...
// Applied to the content groups only (grid/radar cone/route/contacts/ownship) — the crosshair,
// range/mode readouts, and LINK/LOCK footer stay screen-fixed, like a magnifying glass held over
// the picture rather than the panel. ICON_SHRINK 1.0 keeps icons at their normal size while
// zoomed — only the spacing between them grows, which is what actually separates an overlapping
// cluster.
var ZOOM_GROUP_IDS = ['hsd-grid', 'hsd-radar', 'hsd-route', 'hsd-threats', 'hsd-contacts', 'hsd-ownship'];
...
if (typeof module !== 'undefined' && module.exports)
  module.exports = { hsdXY: hsdXY, rangeLabelForTest: function (metric, meters) { state.metric = metric; return rangeLabel(meters); },
                     ...
                     // Cursor-anchored zoom/icon-shrink and pending-selection are shared with
                     // rdr.js via services/cursor-zoom.js and services/pending-selection.js — see
                     // their own test files for that coverage, not here.
