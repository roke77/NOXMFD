...
assert.ok(routed.size > 10, `found too few routes (${routed.size}) in TelemetryServer.cs — regex probably broke`);
...
for (const [layout, table] of [['classic-full', CLASSIC_FULL], ['classic-split', CLASSIC_SPLIT], ['f35', F35]]) {
  for (const [page, url] of Object.entries(table)) {
    ...
    assert.ok(routed.has(base),
      `${layout}.${page} points at '${url}', but TelemetryServer.cs has no 'path == "${base}"' route — ` +
