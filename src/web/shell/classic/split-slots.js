...
(function (root) {
  const SPLIT_SLOTS = {
    ...
    // OBJ above but only 5 items — MAIN+HUD+KEY fill the left column, LYT+RTS spill onto right
    // slots 0..1. Index-aligned with NAV.hud/NAV.keys/NAV.rates.
  ...
  // grouping: MAIN/GRID/FLW/Z+/Z- fill page 1, then WPT/R+/R-/W+/W- fill page 2 — mainPageSizes'
  // fixed 5-then-5 split for 10 items lands exactly on that boundary. An action-name list, not NAV
  // items directly — this module holds no reference to NAV (nav-model.js), so mfd.js maps these
  // onto the real NAV.map items; split-slots.test.js checks the resulting pairing against NAV.map
  // directly, so an edit to NAV.map that breaks the pairing fails there instead of shipping a
  // decorator that can never render.
  //
