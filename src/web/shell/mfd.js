...
function rdrMsg() {
  return { mfd: true, type: 'rdr', present: rdrData.present, range: rdrData.range,
           cone: rdrData.cone, hdg: rdrData.hdg, items: rdrData.items || [] };
}
...
let rdrData = { present: false, range: 0, cone: 0, hdg: 0, items: [] };
...
window.addEventListener('message', function(e) {
  ...
  if (m.type === 'status') {
    ...
    rdrData = { present: !!m.present, range: m.range || 0, cone: m.cone || 0, hdg: m.hdg || 0,
