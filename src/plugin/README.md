...
| [Keybinds.cs](Keybinds.cs) | **Dedicated countermeasure keybinds.** Rebindable `ConfigEntry<KeyboardShortcut>` keys (flares, radar jammer) polled each frame from the reader; each one selects its countermeasure and deploys it directly via `CountermeasureManager.DeployCountermeasure` (tap-pop flares / hold-to-jam), bypassing the stock input's contested deploy trigger so no cycle-then-fire is needed. |
