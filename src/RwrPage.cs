namespace NORoksMFD
{
    ...
    // nose-up radar-warning display drawn as a responsive SVG — a smoked-white mask (rim + two
    // dashed range rings + cardinal ticks + ownship caret + heading caret) with contacts drawn
    // as 50%-opacity bearing spokes from ownship out to each emitter, tier-coloured (grey search
    // / yellow track / red lock). See todo/rwr-radar-warning.md.
    //
    // STUB: contacts are placeholder/fake (FAKE_RWR), rendered once on load — no fetches, no
    // postMessage. When the backend feed lands, drive it from the shell like AVN/TGP (forward an
    // 'rwr' message with the parsed RwrContact[] and re-run renderRwr), the same way the full-view
    // scope will be wired.
    internal static class RwrPage
    {
